...
\end{consequence}
\begin{proof}[Proof of Fubinis Theorem]
    Let $P$ be a partition of the rectangle $A \times B$ in $\R^{n+m}$ and $P = \{P_A,P_b\}$ where $P_A$ is a partition of $A$ and $P_B$ is a partition of $B$, i.e every rectangle $R$ obtained from $P$ can be written as $R = R_A \times R_B$ with $R_A$ obtained from $P_A$ and $R_B$ obtained from $P_B$. \[L(P,f) = \sum_{R = R_A\times R_B} M_{R_A \times R_B}\vol(R_A \times R_B) \] \[\text{ where } m_{R_A \times R_B} = \inf\{f(x,y): x \in R_A, y \in R_B\} \text{ and } \vol(R_A \times R_B) = \vol(R_A)\vol(R_B)\] Fix $y_0 \in R_B$ then \[\inf\{f(x,y): x \in R_A\} \ge m_{R_A \times R_B}\] \[L(P_A,f(\cdot ,y_0)) = \sum_{x \in R_A} \inf \{f(x,y_0): x \in R_A\} \vol(R_A) \ge \sum_{x \in R_A} m_{R_A\times R_B}\vol(R_A)\] and note that \[L(P_A, f(\cdot, y_0)) \le \underline{\int_A}f(x,y_0)dx = \mathcal{L}(y)\] Therefore, \[\mathcal{L}(y_0) \ge \sum_{x\in R_A}m_{R_A \times R_B} \vol(R_A) \quad \forall y_0 \in R_B\] which implies \[\inf{\mathcal{L}(y): y \in R_B} \ge \sum_{x \in R_A} m_{R_A \times R_B}\vol(R_A)\] \[L(P,f) = \sum_{R_B}\sum_{R_A}m_{R_A \times R_B}\vol(R_A)\vol(R_B) \le \sum_{R_B} \inf\{\mathcal{L}(y): y \in R_B\}\vol(R_B) = L(P_B, \mathcal{L})\] So we conclude that \[L(P,f) \le L(P_B, \mathcal{L})\] and similarly  \[U(P,f) \ge U(P_B,\mathcal{L})\] Therefore \[L(P,f) \le L(P_B,\mathcal{L}) \le U(P_B, \mathcal{L}) \le U(P,f)\] Let $\eps >0$ since $f \in R (A \times B)$ we choose $P$ such that \[U(P,f) - L(P,f) < \eps\] and therefore for the corresponding parition $P_B$ of $R_B$ we get \[U(P_B, \mathcal{L}) - L(P_B, \mathcal{L}) \le \eps\] which implies that $\mathcal{L} \in R(B)$ moreover \[L(P,f) \le \int_B \mathcal{L}(y) dy \le U(P,f)\] and $L(P,f) \le \int_{A \times B} f(x,y)d(x,y) \le U(P,f)$ which implies that \[|\int_{A \times B} F(x,y)d(x,y) - \int_B \mathcal(y) dy| < \eps\] for every $\eps >0$ which implies that \[\int_{A \times B} f(x,y)d(x,y) = \int_B \mathcal{L}(y) dy\] similarly we have $\mathcal{U} \in R(B)$ and \[\int_{A \times B} f(x,y)d(x,y) = \int_B \mathcal{U}(y) dy\]
\end{proof}
...
\chapter{The Change of variable formula.} 
\section{Measure zero with cube covering}
\begin{proposition}
...
\section{Case of a Linear transformation}
\begin{remark}
...
\begin{remark}
    ...
    Moreover, notice that if $x \in V$, with $\|x\| = 1$, then $\abs{Tx} \le \abs{T}$. 
